...
def log_policy_event(
    run_dir: Path, sutra_dir: Path, event_type: str, details: str
) -> None:
...
def update_token_ledger(
    ...
    try:
        ...
        if repo_root:
            ...
            # Load marketing config and multiplier from sutra.yaml
            sutra_yaml_path = repo_root / ".sutra" / "sutra.yaml"
    ...
    if is_dirty:
        ...
        stash_res = subprocess.run(
            [
                ...
                "-u",
                "-m",
                f"Sutra auto-stash for merge {mission_id}",
            ],
            cwd=repo_root,
            capture_output=True,
            text=True,
...
def execute_single_task(
    ...
    # Formulate prompt
    agent_file = (
        (worktree_path / ".sutra" if use_worktree else sutra_dir)
        / "agents"
        / f"{task['agent']}.md"
    )
    agent_instruction = ""
...
"""
    ...
    run_hooks(
        "post_task", {"mission_id": mission_id, "task": task_copy}, sutra_dir, console
    )
...
def run_mission_start(
    ...
    if not approved:
        if non_interactive and os.environ.get("SUTRA_CI_AUTO_APPROVE") == "1":
            ...
            console.print(
                "[cyan]Non-interactive mode & SUTRA_CI_AUTO_APPROVE=1: Auto-approving mission...[/]"
            )
            console.print(
            ...
            log_execution_event(
                ...
                "POLICY_WARNING",
                "",
                "Mission auto-approved via SUTRA_CI_AUTO_APPROVE=1 (approval gate bypassed).",
            )
        else:
