...
def _check_niyam_structure(repo_root: Path) -> list[DiagnosticResult]:
    """Check that .niyam/ has the expected structure."""
    results: list[DiagnosticResult] = []
    niyam_dir = repo_root / NIYAM_DIR

    # Required files
    required_files = [NIYAM_YAML, PROJECT_YAML, RUNTIMES_YAML]
...
def _check_yaml_validity(repo_root: Path) -> list[DiagnosticResult]:
    ...
    for yaml_file in niyam_dir.rglob("*.yaml"):
        rel = yaml_file.relative_to(niyam_dir)
        try:
...
def _check_claude_runtime(repo_root: Path) -> list[DiagnosticResult]:
    ...
    if claude_md.exists():
        ...
        results.append(DiagnosticResult("CLAUDE.md", False, "Missing — run niyam sync"))
...
def _check_codex_runtime(repo_root: Path) -> list[DiagnosticResult]:
    ...
    if agents_md.exists():
        ...
        results.append(DiagnosticResult("AGENTS.md", False, "Missing — run niyam sync"))
...
def _check_gemini_runtime(repo_root: Path) -> list[DiagnosticResult]:
    ...
    if gemini_md.exists():
        ...
        results.append(DiagnosticResult("GEMINI.md", False, "Missing — run niyam sync"))
...
def _check_agents_validity(repo_root: Path) -> list[DiagnosticResult]:
    results = []
    agents_dir = repo_root / ".niyam" / "agents"
...
def _run_planner_smoke(engine: str) -> tuple[bool, str]:
    ...
    prompt = "Return exactly this text and nothing else: NIYAM_PLANNER_OK"
    cmd = [engine, "-p", prompt]
    if engine == "gemini":
    ...
    try:
        ...
        return ("NIYAM_PLANNER_OK" in output and res.returncode == 0), output.strip()[
            :100
        ]
    except Exception as e:
        return False, str(e)
...
def _run_claude_smoke() -> tuple[bool, str]:
    ...
    prompt = "Return exactly this text and nothing else: NIYAM_CLAUDE_OK"
    try:
        res = subprocess.run(
            ["claude", "-p", prompt],
            stdin=subprocess.DEVNULL,
        ...
        output = (res.stdout or "") + (res.stderr or "")
        return ("NIYAM_CLAUDE_OK" in output and res.returncode == 0), output.strip()[
            :100
        ]
    except Exception as e:
        return False, str(e)
...
def run_doctor(
    ...
    """Run diagnostic checks on the Niyam workspace."""
    import os
    ...
    # Core checks
    if runtime is None:
        all_results.extend(_check_niyam_structure(root))
    ...
    # Display results
    table = Table(title="Niyam Doctor", show_lines=False)
