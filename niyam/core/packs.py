"""Niyam pack manager — manage modular skill/agent/command bundles."""
...
def add_pack(
    ...
    for p in pack_dir.rglob("*"):
        if p.is_file() and p.name != "pack.yaml":
            ...
            target_path = niyam_dir / rel_path
...
def remove_pack(repo_root: Path, name: str, console: Console | None = None) -> None:
    ...
    niyam_dir = get_niyam_dir(repo_root)
    if not niyam_dir.exists():
        raise FileNotFoundError("Niyam workspace is not initialized.")
...
def sync_packs(repo_root: Path, console: Console | None = None) -> None:
    """Sync all packs listed in niyam.yaml by re-applying them."""
    config = load_niyam_config(repo_root)
