...
def _boilerplate_cli_commands(root: Path, console: Console) -> None:
    ...
    commands_dir = root / ".niyam" / "commands"
    ...
    def traverse(cmd, parts: list[str]):
        ...
        if isinstance(cmd, typer.core.TyperGroup):
            ...
            if not target_file.exists():
                ...
                console.print(
                    f"  [dim]•[/] Boilerplated missing command template: [cyan].niyam/commands/{filename}[/]"
                )
...
def run_context_refresh(console: Console) -> None:
    ...
    project_yaml_path = root / ".niyam" / "project.yaml"
    project_data: dict = {}
    ...
    # Auto-boilerplate CLI commands under .niyam/commands/
    try:
    ...
    console.print(
        Panel(
            ...
            "\n"
            "[dim]Run [bold]niyam sync[/] to propagate changes to runtimes.[/]",
