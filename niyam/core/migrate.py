...
def run_sutra_migration(force: bool = False, move: bool = False, console: Console = None) -> None:
    ...
    if not sutra_dir:
        con.print("[bold red]Error:[/] No legacy .sutra/ directory found in this repository or its parent directories.")
        raise SystemExit(1)
    ...
    if niyam_dir.exists():
        if not force:
            con.print(f"[bold red]Error:[/] Destination directory {niyam_dir} already exists.")
    ...
    for path in all_files:
        if path.is_file():
            ...
            try:
                ...
                con.print(f"[yellow]Warning:[/] Could not update contents of {path.name}: {e}")
    ...
    if move:
        try:
            ...
            con.print(f"[yellow]Warning:[/] Could not remove old .sutra/ directory: {e}")
