...
class ClaudeAdapter(RuntimeAdapter):
    ...
    def sync(self, console: Console) -> None:
        """Generate Claude Code files from .niyam/ source of truth."""
    ...
    def _generate_claude_md(self, console: Console) -> None:
        """Generate CLAUDE.md from .niyam/context/ and .niyam/policies/."""
        sections: list[str] = []
    ...
    def _build_policies_section(self) -> str:
        ...
        if cmd_policy:
            ...
            if deny:
                ...
                lines.append("The following commands are **blocked** by Niyam policy:")
                for cmd in deny:
                    lines.append(f"- `{cmd}`")
                lines.append("")
    ...
    def _project_agents(self, console: Console) -> None:
        """Copy .niyam/agents/ → .claude/agents/."""
        source_dir = self.niyam_dir / "agents"
    ...
    def _project_commands(self, console: Console) -> None:
        """Copy .niyam/commands/ → .claude/commands/."""
        source_dir = self.niyam_dir / "commands"
        target_dir = self.repo_root / ".claude" / "commands"
        self._mirror_directory(source_dir, target_dir, console)

    def _project_skills(self, console: Console) -> None:
        """Copy .niyam/skills/ → .claude/skills/."""
        source_dir = self.niyam_dir / "skills"
