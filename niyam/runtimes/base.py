...
def check_command(command):
    ...
    for pattern in DENY_PATTERNS:
        if _matches_pattern(command_lower, pattern):
            ...
            return {{
                "allowed": False,
                "reason": f"Blocked by Niyam policy: '{{pattern}}' is in the deny list.",
            }}
    ...
    for pattern in WARN_PATTERNS:
        if _matches_pattern(command_lower, pattern):
            ...
            return {{
                "allowed": True,
                "warning": f"Niyam caution: '{{pattern}}' - proceed carefully.",
            }}
...
def check_file_path(file_path):
    ...
    if DENY_WRITE_PATTERNS and any(
        ...
        return {{
            "allowed": False,
            "reason": f"Blocked by Niyam security policy: file '{{file_path}}' matches deny_write_patterns.",
        }}
    ...
    if ALLOW_WRITE_PATTERNS and not any(
        ...
        return {{
            "allowed": False,
            "reason": f"Blocked by Niyam security policy: file '{{file_path}}' does not match allow_write_patterns.",
        }}
    ...
    if FROZEN_PATHS:
        ...
        for allowed_pat in FROZEN_PATHS:
            if fnmatch.fnmatch(file_path, allowed_pat) or file_path.startswith(
                allowed_pat
            ):
                return {{"allowed": True}}
        ...
        return {{
            "allowed": False,
            "reason": f"Blocked by Niyam guard: file '{{file_path}}' is outside the allowed scope: {{FROZEN_PATHS}}",
        }}
