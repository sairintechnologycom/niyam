...
def generate_markdown_report(results: dict) -> str:
    ...
    if not findings:
        ...
        for f in findings:
            ...
            lines.append(f"| {f['id']} | **{f['severity'].upper()}** | {f['category']} | {f['description']} | {fpath} |")
...
def scan_command(
    path: Annotated[
        str,
        typer.Argument(help="Path to scan (defaults to current directory).")
    ] = ".",
    profile: Annotated[
        str,
        typer.Option("--profile", "-p", help="Scan profile: startup, team, enterprise.")
    ] = "startup",
    output: Annotated[
        str,
    ...
    report_file: Annotated[
        str,
        typer.Option("--report-file", "-f", help="Output file path to save the report.")
    ] = None,
    rules: Annotated[
        str,
        typer.Option("--rules", help="Path to custom rules YAML file.")
    ] = None,
) -> None:
    ...
    results = run_scanner_checks(scan_path, profile=profile, custom_rules_path=custom_rules)
    ...
    if not findings:
        console.print("[bold green]✓ No findings! Your repository is production-ready.[/]")
    else:
    ...
    console.print(Panel(panel_content, title="[bold]Readiness Summary[/]", border_style="cyan" if score >= 70 else "red"))
