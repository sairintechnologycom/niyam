...
class TestGuard:
    ...
    def test_guard_enable(self, sutra_repo: Path) -> None:
    ...
    def test_guard_disable(self, sutra_repo: Path) -> None:
        """guard disable should set guard.enabled to False."""
        from sutra.policies.guard import run_guard_enable, run_guard_disable
    ...
    def test_guard_careful_adds_warn_list(self, sutra_repo: Path) -> None:
        ...
        with open(sutra_repo / ".sutra" / "policies" / "commands.yaml") as f:
            data = yaml.safe_load(f)

        assert len(data.get("warn", [])) > 0
        assert "rm -rf" in data["warn"]

    def test_guard_freeze_adds_path(self, sutra_repo: Path) -> None:
    ...
    def test_guard_freeze_enables_guard(self, sutra_repo: Path) -> None:
