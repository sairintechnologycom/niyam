...
def test_scan_clean_app(tmp_path: Path) -> None:
    ...
    (tmp_path / "README.md").write_text("# Clean App\nThis is a production-ready application.")
    ...
    assert len(results["findings"]) == 0 or not any(f["severity"] in ("critical", "high") for f in results["findings"])
...
def test_scan_risky_app_env_no_go(tmp_path: Path) -> None:
    ...
    (tmp_path / ".env").write_text("OPENAI_API_KEY=sk-proj-1234567890abcdef1234567890abcdef\n")
...
def test_scan_missing_readme(tmp_path: Path) -> None:
    ...
    readme_findings = [f for f in findings if f["category"] == "docs" or f["id"] == "DOC001"]
...
def test_scan_missing_tests(tmp_path: Path) -> None:
    ...
    findings = results["findings"]
    test_findings = [f for f in findings if f["category"] == "tests" or f["id"] == "TST001"]
...
def test_scan_ai_risk_placeholder(tmp_path: Path) -> None:
    ...
    findings = results["findings"]
    ai_findings = [f for f in findings if f["category"] == "ai_risk" or f["id"] in ("AI001", "AI002")]
...
def test_scan_cli_redacted_outputs(tmp_path: Path) -> None:
    ...
    (secret_workspace / ".env").write_text("OPENAI_KEY=sk-proj-1234567890abcdef1234567890abcdef\n")
    ...
    # 2. JSON output
    result_json = runner.invoke(app, ["scan", str(secret_workspace), "--output", "json"])
