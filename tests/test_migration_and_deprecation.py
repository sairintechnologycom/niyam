...
def test_python_m_niyam_help() -> None:
    ...
    assert (
        "Governed autonomous development for AI coding agents" in res.stdout
        or "niyam" in res.stdout.lower()
    )
...
def test_niyam_migration_lifecycle(tmp_repo: Path) -> None:
    ...
    agent_file.write_text(
        "Sutra backend specialist policies. Check `.sutra/context/`.", encoding="utf-8"
    )
