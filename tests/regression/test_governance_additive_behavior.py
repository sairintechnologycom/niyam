...
def test_generated_niyam_files_are_isolated(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["scan", str(tmp_path), "--output", "json"], env={"NIYAM_ROOT": str(tmp_path)})
    ...
    post_files = sorted([p for p in tmp_path.rglob("*") if ".niyam" not in p.parts and p != output_report])
